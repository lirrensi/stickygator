...
export class KeyManager {
    ...
    async encryptData(data: any = {}, options = { asString: true }) {
    ...
    async decryptData(input: Object | string) {
        let encMessage: EncryptedMessage;
        if (typeof input === "string") {
            let inputObj = JSON.parse(input);
            encMessage = {
                format: inputObj.format,
                iv: new Uint8Array(inputObj.iv),
                data: base64.decode(inputObj.data),
            };
        } else {
            encMessage = input as EncryptedMessage;
        }
        try {
            // Decrypt the data
            const decryptedData = await crypto.subtle.decrypt(
                { name: "AES-GCM", iv: encMessage.iv },
                this.key,
                encMessage.data,
            );
            // Unpack the decrypted data using MessagePack
            const unpackedData = msgpack.decode(decryptedData);
            return unpackedData;
        } catch (error) {
            console.error("Decryption error:", error);
            return null;
...
// data -> msgpack -> encrypt -> base64
// file -> chunk -> gzip -> encrypt -> base64
